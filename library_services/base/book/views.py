...
def edit(book_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(book)
        db.session.commit()
        flash(u'Informacje o książce zostały zapisane!', 'success')
    ...
    return render_template("book_edit.html", form=form, book=book, title=u"Edytuj informacje o książce")
...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Książka %s została dodana do biblioteki!' % new_book.title, 'success')
        return redirect(url_for('book.detail', book_id=new_book.id))
    return render_template("book_edit.html", form=form, title=u"Dodaj nową książkę")
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'Pomyślnie usunięto książkę, użytkownik nie może już jej wyświetlić', 'info')
...
def put_back(book_id):
    ...
    db.session.add(the_book)
    db.session.commit()
    flash(u'Książka została pomyślnie przywrócona i użytkownik może teraz wyświetlić książkę', 'info')
    return redirect(request.args.get('next') or url_for('book.detail', book_id=book_id))
