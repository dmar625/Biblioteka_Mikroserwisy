...
class User(UserMixin, db.Model):
    ...
    @property
    def password(self):
        raise AttributeError('hasło jest atrybutem nie do odczytu')
    ...
    def __init__(self, **kwargs):
        ...
        if self.role is None:
            if self.email.lower() == current_app.config['FLASK_ADMIN'].lower():
    ...
    def borrow_book(self, book):
        if self.logs.filter(Log.returned == 0, Log.return_timestamp < datetime.now()).count() > 0:
            return False, u"wypożyczenie nie jest możliwe, zaległe książki nie zostały zwrócone"
        if self.borrowing(book):
            return False, u'książka jest już wypożyczona!'
        if not book.can_borrow():
            return False, u'wszystkie egzemplarze tej książki zostały wypożyczone'

        db.session.add(Log(self, book))
        return True, u'Udało Ci się wypożyczyć książkę %s' % book.title

    def return_book(self, log):
        if log.returned == 1 or log.user_id != self.id:
            return False, u'nie znaleziono książki'
        log.returned = 1
        log.return_timestamp = datetime.now()
        db.session.add(log)
        return True, u'zwróciłeś książkę %s' % log.book.title
